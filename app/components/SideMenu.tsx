...
export default function SideMenu() {
    ...
    const checked = () => {
        ...
        menuRef.current?.classList?.toggle('on');  // Toggle the 'hidden' class
        ...
        for (let i = 0; i < children?.length; i++) {
            console.log(children[i]); // Access each child element
    ...
    return (
        <div>
            ...
            <div ref={menuRef} className="meSlide on md:hidden">
                <p className={`animation ${menuOn?'on':'off'}`}>Home</p>
                <p className={`animation ${menuOn?'on':'off'}`}>Businesses</p>
                <p className={`animation ${menuOn?'on':'off'}`}>Events & Tickets </p>
                <p className={`animation ${menuOn?'on':'off'}`}>Products</p>
                <p className={`animation ${menuOn?'on':'off'}`}>Login</p>
            </div>
        </div>
    )
}
